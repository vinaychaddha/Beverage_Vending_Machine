...
unsigned char Secs,Pre_brew_time;
unsigned char Kbd_pos,Max_iic_bytes,Repeat_key_cntr,Parameter,Dbnc_key_code;
unsigned char Parameter_val[MAX_PARA+1],Ascii_unit,Ascii_ten,Ascii_hun;


//unsigned char Premix1_timeout,Premix2_timeout,Premix3_timeout;
unsigned char Premix_timeout;
unsigned char Water_timeout;
unsigned char Water1_timeout,Water2_timeout,Water3_timeout,Rly_on_dly_tout;
unsigned char Refil_timeout;
unsigned char Flush_timeout;
unsigned char Delay_time;
unsigned char Brew_time;
#define COUNTER_LENGTH	4			// This indicates 8 bcd bytes for counter.
// byte for counter
unsigned char Counter_val[COUNTER_LENGTH],Counter;
...
//#pragma DATA_SEG DEFAULT_RAM
 ...
 unsigned char Msg_arr_index;		// Ranges from MIN_MSG to MAX_MSG-1
*/

// FROM IIC
 unsigned char Temp, Tmp_cntr_iic,Iic_counter,Iic_buff[4]; 
...
unsigned char Key_scan_cntr,Key_code,Dbnc_cntr,i;
unsigned char Dbnc_key_code;


// from timer
unsigned char Ticks, Small_ticks;
